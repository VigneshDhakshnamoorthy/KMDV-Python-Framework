...
class DominosProductPage(DominosHomePage):
    def __init__(self, browser):
        ...
        try:
            ...
            with open(
                f"{os.getcwd()}/Repository/ObjectRepository/products.ini", "r"
            ) as f:
                self.config.read(
                    f"{os.getcwd()}/Repository/ObjectRepository/products.ini"
                )
    ...
    def add_cart_product(self, product_name):
        ...
        product_by = (
            By.XPATH,
            f"(//div[@data-label='{product_name}']//child::button[@data-label='addTocart'])[1]",
        )
        ...
        print(
            f"Added to Cart - {product_name} "
            f"| Qty - {self.get_text(product_qty)} | Price - {self.get_text(product_price)}"
        )

    def cart_value_verify(self):
        each_product_value_ele = self.wait_visibility_all(
            "DominosProductPage", "EachProductPrice_xpath"
        )
        ...
        if sum(each_product_value_list) == float(
            self.get_text("DominosProductPage", "SubTotalPrice_xpath")
        ):
            print(
                "Each Product Total Value ( "
                + str(sum(each_product_value_list))
                + " ) == Sub Total Value ( "
                + self.get_text("DominosProductPage", "SubTotalPrice_xpath")
                + " )"
            )
        else:
            assert sum(each_product_value_list) == float(
                self.get_text("DominosProductPage", "SubTotalPrice_xpath")
            )
